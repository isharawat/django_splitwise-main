    ...
    if request.method=='POST':
       if 'involved' in request.POST:
          ...
          if involved_form.is_valid():
             ...
             return HttpResponseRedirect('/splitwise/transaction/forms/')
    else:
       involved_form=InvolvedForm(final_choices)
...
def transaction_form_multiple(request):
    me = request.user
    ...
    if request.method == 'POST':
       ...
       transaction_form_multiple = TransactionForm(final_choices)

    context ={
       'transaction_form_multiple' : transaction_form_multiple
    }
    return HttpResponse(template.render(context,request))

def transaction_form(request, f):
	me = request.user
	p = User.objects.get(id=f)
	final_choices = ()
	final_choices = final_choices + ((me.username, me.username),)
